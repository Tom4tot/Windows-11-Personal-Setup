...
- Reboot and install Windows. Make sure to delete all your partitions (except the data one, if you have one) and to choose the Education/Enterprise/Professional edition. You can also pick `English (World)` as `the time and currency format` so [most user apps won't be installed](https://www.reddit.com/r/Windows11/comments/15gk07n/english_world_as_time_and_currency_for_debloating/). You can always run `wsreset -i` via CMD (with admin rights) to reset/reinstall Microsoft Store. Also, don't forget to copy your Wi-Fi drivers on your USB stick (after setting up with Rufus) so you can easily access your network once the Windows installation is complete.
